...
const getMe = async (req, res) => {
    try {
        const userDoc = await User.findById(req.user._id).populate('services');
        if (!userDoc) return res.status(404).json({ message: 'User not found in system' });
        ...
        res.status(500).json({ message: 'Identity retrieval failed', error: error.message });
    }
};
