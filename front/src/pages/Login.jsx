        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6">
          <div className="space-y-2">
            ...
            <div className="relative group">
              <Mail size={18} className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${formik.touched.email && formik.errors.email ? 'text-red-400' : 'text-slate-300 group-focus-within:text-saloon-400'}`} />
              <input 
                name="email"
                type="email" 
                {...formik.getFieldProps('email')}
                placeholder="sia@saloon.com"
                className={`w-full bg-white/50 border p-4 pl-12 rounded-2xl outline-none focus:bg-white transition-all font-bold text-slate-700 placeholder:text-slate-300 ${formik.touched.email && formik.errors.email ? 'border-red-200' : 'border-white focus:border-saloon-200'}`} 
              />
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="relative group">
              <Lock size={18} className={`absolute left-4 top-1/2 -translate-y-1/2 transition-colors ${formik.touched.password && formik.errors.password ? 'text-red-400' : 'text-slate-300 group-focus-within:text-saloon-400'}`} />
              <input 
                name="password"
                type="password" 
                {...formik.getFieldProps('password')}
                placeholder="••••••••"
                className={`w-full bg-white/50 border p-4 pl-12 rounded-2xl outline-none focus:bg-white transition-all font-bold text-slate-700 ${formik.touched.password && formik.errors.password ? 'border-red-200' : 'border-white focus:border-saloon-200'}`} 
              />
            </div>
