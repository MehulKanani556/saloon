      ...
      <div className="flex flex-col gap-6 md:gap-8 min-h-[70vh]">
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 py-4 border-b border-white/[0.05]">
          <div className="space-y-1 text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-2">
              ...
              <p className="text-[9px] font-black text-primary uppercase tracking-[0.4em]">Bespoke Concierge</p>
            </div>
            <h1 className="text-xl md:text-2xl font-black text-white uppercase tracking-wide font-luxury">
              Client <span className="text-primary">Reservations</span>
            </h1>
          </div>
          <Link
            to="/book"
        ...
        {loading && appointments.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-32 gap-6">
            ...
            <p className="text-[9px] font-black uppercase tracking-[0.5em] text-muted/40">Syncing Portfolio Matrix...</p>
          </div>
          ...
          >
            ...
            <h3 className="text-xl font-black text-white uppercase tracking-tight mb-3 font-luxury">Archive Null</h3>
            <p className="text-[10px] font-bold text-muted/40 uppercase tracking-[0.2em] mb-8">No active reservations were discovered in the collective.</p>
                    ...
                    <div className="flex-1 p-5 md:p-8 space-y-5">
                      ...
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-6 pt-2">
                        <div className="space-y-0.5">
                          <p className="text-[8px] font-black text-white/20 uppercase tracking-widest">Investment</p>
                          <p className="text-xl font-black text-primary font-luxury drop-shadow-primary-sm">${app.totalPrice?.toFixed(2)}</p>
                        </div>
                        <div className="space-y-0.5">
                          <p className="text-[8px] font-black text-white/20 uppercase tracking-widest">Time Sync</p>
                        ...
                        <div className="space-y-0.5 col-span-2 md:col-span-1">
                          ...
                          <p className="text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2 pt-1 border-t border-white/[0.03] md:border-0">
                            <Scissors size={12} className="text-primary/40" /> Luxury Ritual
                          </p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="w-full lg:w-72 xl:w-80 p-5 md:p-8 bg-white/[0.01] flex flex-col justify-between gap-6">
                      <div className="space-y-3">
                        <h4 className="text-[9px] font-black text-white/20 uppercase tracking-widest flex items-center gap-2">
                          <Scissors size={12} /> Ritual Sequence
                        </h4>
                        <div className="space-y-2 text-[10px] font-black uppercase tracking-wide">
                          ...
                          {app.assignments?.length > 2 && (
                            <p className="text-[9px] font-black text-primary/40 italic tracking-widest">+{app.assignments.length - 2} Sequential Rituals</p>
                          )}
                        </div>
                      </div>
                      ...
                      <button className="w-full py-3.5 bg-white/[0.03] border border-white/5 text-[9px] font-black uppercase tracking-widest text-white hover:bg-primary hover:text-secondary hover:border-primary transition-all duration-300 rounded-xl flex items-center justify-center gap-2 group/btn shadow-lg">
                        Reservation Detail <ChevronRight size={12} className="group-hover/btn:translate-x-0.5 transition-transform" />
                      </button>
                    </div>
                ...
                <div className="flex justify-between items-start mb-6 md:mb-8 pb-6 border-b border-white/[0.05]">
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em]">Protocol Retrieval</p>
                    </div>
                    <h2 className="text-xl sm:text-3xl font-black text-white uppercase tracking-tight font-luxury">Reservation Details</h2>
                ...
                <div className="space-y-8 max-h-[65vh] overflow-y-auto scrollbar-hide pr-1 md:pr-2">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6 bg-white/[0.02] p-5 md:p-6 rounded-2xl md:rounded-3xl border border-white/[0.03]">
                    <div className="space-y-1.5">
                      <p className="text-[9px] font-black text-white/20 uppercase tracking-widest">Reserved Schedule</p>
                    ...
                    <div className="space-y-2">
                      <p className="text-[9px] font-black text-white/20 uppercase tracking-widest">Protocol Status</p>
                  ...
                  <div className="space-y-5">
                    <h4 className="text-[10px] font-black text-primary uppercase tracking-[0.3em] flex items-center gap-3">
                      <Scissors size={14} /> Ritual Breakdown
                    </h4>
                    <div className="space-y-3">
                      {selectedApp.assignments?.map((asm, i) => (
                        <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 bg-white/[0.02] border border-white/[0.03] rounded-2xl group hover:border-primary/20 transition-all gap-3">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="space-y-0.5">
                              ...
                              <p className="text-[9px] font-bold text-muted/30 uppercase tracking-widest">
                                Artisan: <span className="text-primary/60">{asm.staff?.name || 'Assigned'}</span>
                              </p>
                            </div>
                          </div>
                          <div className="sm:text-right border-t sm:border-0 border-white/[0.03] pt-3 sm:pt-0">
                            ...
                            <p className="text-[8px] font-bold text-muted/20 uppercase tracking-widest tabular-nums">{asm.service?.duration} Mins Ritual</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 sm:gap-0">
                      <div className="space-y-1">
                        <p className="text-[9px] font-black text-white/20 uppercase tracking-widest flex items-center gap-2">
                           <Clock size={12} className="text-primary/40" /> Global Sync
                        </p>
                        <p className="text-xl font-black text-white uppercase tracking-tighter">
                          {format(new Date(selectedApp.appointmentDate), 'hh:mm a')} <span className="text-muted/20 text-xs">UTC+</span>
                        </p>
                      </div>
                      <div className="sm:text-right space-y-1">
                        <p className="text-[9px] font-black text-white/20 uppercase tracking-widest">Total Investment</p>
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-center p-4">
            ...
            >
              <div className="p-6 md:p-8 text-center">
                ...
                <h3 className="text-xl font-black text-white uppercase tracking-tight mb-2 font-luxury">Terminate Protocol?</h3>
                <p className="text-[10px] font-bold text-muted/50 uppercase tracking-[0.1em] leading-relaxed mb-6">
                  Warning: This action permanently deletes your reservation. <span className="text-red-400">Time slot restoration is not guaranteed.</span> 
                </p>
