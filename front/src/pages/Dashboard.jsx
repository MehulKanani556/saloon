  ...
  if (loading) return (
    <div className="flex items-center justify-center h-[70vh]">
      <div className="relative">
        <Loader2 className="w-16 h-16 text-parlour-600 animate-spin" />
        <Brain className="w-6 h-6 text-parlour-500 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2" />
      </div>
    </div>
  );
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-8">
        {[
          ...
          <motion.div
            ...
            className="group bg-white dark:bg-slate-900 p-6 md:p-8 rounded-2xl border border-slate-50 dark:border-white/5 shadow-2xl hover:shadow-parlour-500/10 transition-all relative overflow-hidden"
          >
            ...
            <div className="flex items-start justify-between relative z-10 mb-6 md:mb-8">
              <div className={`w-12 h-12 md:w-14 md:h-14 rounded-2xl bg-slate-50 dark:bg-slate-800 flex items-center justify-center text-slate-400 group-hover:bg-parlour-500 group-hover:text-white transition-all duration-500 shadow-sm`}>
              ...
              <AreaChart data={data.financialVelocity}>
                <defs>
                  <linearGradient id="colorRev" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#ff3d9f" stopOpacity={0.25} />
                ...
                <Tooltip
                  cursor={{ stroke: '#ff3d9f', strokeWidth: 2, strokeDasharray: '4 4' }}
                  contentStyle={{ backgroundColor: '#0f172a', border: 'none', borderRadius: '24px', boxShadow: '0 25px 50px -12px rgba(0, 0, 0, 0.5)', padding: '20px' }}
                  itemStyle={{ color: '#ff3d9f', fontWeight: 900, textTransform: 'uppercase', fontSize: '12px', fontStyle: 'italic' }}
            ...
            {data.recentBookings.map((app, i) => (
              <div key={app._id} className="flex items-center justify-between p-5 rounded-2xl bg-slate-50/50 dark:bg-slate-800/20 border border-slate-100/50 dark:border-white/5">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-xl bg-parlour-500/10 flex items-center justify-center text-parlour-500">
          ...
          <div className="space-y-4 md:space-y-6 relative z-10">
            {data.upcomingRituals.length > 0 ? data.upcomingRituals.map((app, i) => (
              ...
              <div className="p-10 text-center border-2 border-dashed border-slate-100 dark:border-white/5 rounded-[2rem]">
                <CheckCircle2 size={32} className="mx-auto text-parlour-500/30 mb-4" />
        ...
        <div className="h-[250px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.occupancyTrends}>
              ...
              <Tooltip
                cursor={{ fill: 'rgba(255, 61, 159, 0.05)' }}
                contentStyle={{ backgroundColor: '#0f172a', border: 'none', borderRadius: '16px', color: '#fff' }}
                itemStyle={{ color: '#ff3d9f', fontWeight: 900, textTransform: 'uppercase', fontSize: '10px' }}
              />
              <Bar dataKey="intensity" radius={[4, 4, 0, 0]} barSize={24}>
                {data.occupancyTrends.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={entry.intensity > 1 ? '#ff3d9f' : '#6366f1'} opacity={0.8} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
