        ...
        <div className="space-y-12 pb-20">
                    ...
                    <div className="flex flex-col sm:flex-row gap-6">
                        {!isAdmin && (
                            <button
                                ...
                                className={`flex items-center gap-4 px-10 py-5 rounded-2xl font-black uppercase text-xs tracking-[0.4em] shadow-xl transition-all group font-luxury ${hasPendingRequest ? 'bg-secondary text-muted cursor-not-allowed border border-white/5' : 'bg-primary text-secondary shadow-primary/20 hover:scale-[1.05]'}`}
                            >
            ...
            {!isAdmin && (
                <div className="space-y-8">
                    <div className="flex items-center justify-between">
                         <div>
                            <h3 className="text-xl font-black text-white uppercase tracking-tighter font-luxury">Current Expertise Portfolio</h3>
                            <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em] mt-1">Verified Ritual Alignment</p>
                        </div>
                    </div>
                    ...
                    {userInfo?.specialization?.length > 0 && (
                        <div className="flex flex-wrap gap-3 pb-4">
                            {userInfo.specialization.map(tag => (
                                <span key={tag} className="px-5 py-2 bg-primary/10 border border-primary/20 text-primary rounded-xl text-[9px] font-black uppercase tracking-widest ">
                                    {tag}
                                </span>
                            ))}
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8 grayscale-[0.2]">
                        {userInfo?.services?.map((service) => (
                            <div key={service._id} className="relative p-8 rounded-2xl border border-white/5 bg-secondary/30 backdrop-blur-sm overflow-hidden group">
                                <div className="relative z-10 flex flex-col gap-6 ">
                                    <div className="flex justify-between items-start">
                                        <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-background border border-white/5 text-primary">
                                            <Scissors size={20} strokeWidth={2.5} />
                                        </div>
                                        <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 rounded-xl text-[9px] font-black uppercase tracking-widest ">
                                            <CheckCircle2 size={12} /> ESTABLISHED
                                        </div>
                                    </div>
                                    <div>
                                        <p className="text-[9px] font-black text-primary uppercase tracking-[0.4em] mb-2 ">{service.category?.name || 'MASTERPIECE'}</p>
                                        <h3 className="text-lg font-black text-white font-luxury uppercase tracking-tight">{service.name}</h3>
                                    </div>
                                </div>
                            </div>
                        ))}
                    ...
                    <table className="w-full text-left">
                        <thead>
                            <tr className="bg-background/80">
                                <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.4em] text-primary">Artisan</th>
                                <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.4em] text-primary">Delta Logic</th>
                                <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.4em] text-primary">Alignment</th>
                                <th className="px-10 py-5 text-[10px] font-black uppercase tracking-[0.4em] text-primary">Status</th>
                                <th className="px-10 py-5 text-center text-[10px] font-black uppercase tracking-[0.4em] text-primary">Authority</th>
                            </tr>
                        </thead>
                            ...
                            {filteredRequests.map(req => {
                                ...
                                return (
                                    <tr key={req._id} className="hover:bg-white/5 transition-colors group">
                                            ...
                                            {isAdmin ? (
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl overflow-hidden border border-white/5">
                                                        <img src={req.staff?.profileImage ? (req.staff.profileImage.startsWith('http') ? req.staff.profileImage : `http://localhost:5000/uploads/profile pic/${req.staff.profileImage}`) : `https://api.dicebear.com/7.x/avataaars/svg?seed=${req.staff?.name}`} className="w-full h-full object-cover" />
                                                    </div>
                                                    <span className="text-[11px] font-black text-white uppercase tracking-widest">{req.staff?.name}</span>
                                                </div>
                                            ) : (
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl overflow-hidden border border-white/5 bg-primary/10 flex items-center justify-center text-primary">
                                                        <LayoutDashboard size={18} />
                                                    </div>
                                                    <span className="text-[11px] font-black text-white uppercase tracking-widest">Self Identification</span>
                                                </div>
                                            )}
                                        </td>
                                        <td className="px-10 py-6">
                                        ...
                                        <td className="px-10 py-6">
                                            <span className="text-[10px] font-black text-primary/60 uppercase tracking-widest">{req.services?.length} TOTAL PROTOCOLS</span>
                                        </td>
                                        <td className="px-10 py-6">
                                            <div className={`inline-flex px-4 py-2 rounded-xl text-[9px] font-black uppercase tracking-widest border ${req.status === 'Approved' ? 'bg-emerald-500/10 text-emerald-500 border-emerald-500/20' : req.status === 'Rejected' ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' : 'bg-amber-500/10 text-amber-500 border-amber-500/20'}`}>
                                                {req.status}
                                            </div>
                                            {req.status === 'Rejected' && req.adminReason && (
                                                <p className="text-[9px] text-rose-500 font-bold tracking-widest mt-2 uppercase">{req.adminReason}</p>
                                            )}
                                        </td>
                                        <td className="px-10 py-6 text-center">
                                            {isAdmin && req.status === 'Pending' ? (
                                                <button onClick={() => { setSelectedRequest(req); setIsActionModalOpen(true); }} className="p-3 bg-primary text-secondary rounded-xl hover:scale-110 transition-transform shadow-premium">
                                                    <LayoutDashboard size={18} />
                                                </button>
                                            ) : <p className="text-[9px] font-black text-muted/20 uppercase tracking-widest">Complete</p>}
                                        </td>
                                    </tr>
                                );
                            })}
            ...
            {isStaffAddModalOpen && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 sm:p-12">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="bg-secondary relative z-20 w-full max-w-4xl rounded-3xl border border-white/5 shadow-3xl overflow-hidden p-12">
                        <div className="flex items-center justify-between mb-12">
                            <div>
                                <h3 className="text-3xl font-black text-white uppercase tracking-tighter font-luxury">Propose Ritual Alignment</h3>
                                <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em] mt-3">Expansion of Architectural Expertise</p>
                            </div>
                            <button onClick={() => setIsStaffAddModalOpen(false)} className="p-4 rounded-2xl bg-white/5 text-muted hover:text-white transition-all"><X size={20} /></button>
                        </div>

                        <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 max-h-[60vh] overflow-y-auto pr-4 custom-scrollbar pb-8">
                                            ...
                                            >
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div className="flex flex-col">
                                                         ...
                                                         <span className="text-[8px] font-black text-primary uppercase tracking-[0.3em]">${service.price}</span>
                                                    </div>
                                                </div>
                                                {isAlreadyEstablished && <div className="absolute top-2 right-2 px-2 py-1 bg-emerald-500/20 text-emerald-500 rounded text-[7px] font-black uppercase">Established</div>}
                                            </div>
                                        );
                                    })}
                                </div>
                            ...
                            <div className="lg:col-span-2 space-y-10">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <input 
                                        ...
                                        className="w-full bg-background border border-white/5 focus:border-primary/30 rounded-xl px-6 py-4 text-[10px] font-black uppercase tracking-widest outline-none transition-all text-white shadow-inner placeholder:text-white/5"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="pt-10 border-t border-white/5 mt-auto">
                            <button 
                                ...
                                className="w-full py-6 bg-primary text-secondary rounded-2xl font-black uppercase text-[11px] tracking-[0.5em] shadow-2xl shadow-primary/20 hover:scale-[1.02] active:scale-[0.98] transition-all font-luxury group"
                            > 
                                COMMIT SYNCHRONIZATION REQUEST
                                <Sparkles size={18} className="inline ml-4 group-hover:rotate-12" />
                            </button>
                        </div>
                    </motion.div>
                </div>
            ...
            {isAdmin && isActionModalOpen && selectedRequest && (
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 sm:p-12">
                    ...
                    <motion.div initial={{ opacity: 0, scale: 0.95, y: 30 }} animate={{ opacity: 1, scale: 1, y: 0 }} className="bg-secondary relative z-20 w-full max-w-2xl rounded-3xl border border-white/5 shadow-3xl overflow-hidden p-12">
                        <div className="flex items-center justify-between mb-12">
                            <div>
                                <h3 className="text-3xl font-black text-white uppercase tracking-tighter font-luxury">Delta Identity Authority</h3>
                                <p className="text-[10px] font-black text-primary uppercase tracking-[0.4em] mt-3">Verifying {selectedRequest.staff?.name}'s New Service Additions</p>
                            </div>
                            <button onClick={() => setIsActionModalOpen(false)} className="p-4 rounded-2xl bg-white/5 text-muted hover:text-white transition-all"><X size={20} /></button>
                        </div>
                        <div className="space-y-12 mb-12 max-h-[60vh] overflow-y-auto pr-4 custom-scrollbar">
                        ...
                        <div className="grid grid-cols-2 gap-8 pt-8 border-t border-white/5">
                            <button onClick={() => handleAdminAction(selectedRequest._id, 'Approved')} className="py-5 bg-emerald-500 text-white rounded-2xl font-black uppercase text-[11px] tracking-[0.4em] shadow-xl shadow-emerald-500/20 active:scale-95 font-luxury"> Approve Rituals </button>
                            <button onClick={() => handleAdminAction(selectedRequest._id, 'Rejected')} className="py-5 bg-rose-500 text-white rounded-2xl font-black uppercase text-[11px] tracking-[0.4em] shadow-xl shadow-rose-500/20 active:scale-95 font-luxury"> Reject Protocol </button>
                        </div>
                    </motion.div>
                </div>
            )}
