...
export default function Settings() {
  ...
  if (loading && !settings) return (
    <div className="flex items-center justify-center h-[60vh]">
      <Loader2 className="w-12 h-12 text-parlour-600 animate-spin" />
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        <div className="lg:col-span-2 space-y-10">
          <div className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-2xl border border-slate-50 dark:border-white/5 shadow-2xl relative overflow-hidden group">
            <div className="absolute top-0 left-0 w-2 h-full bg-parlour-600" />
            <h3 className="text-lg md:text-xl font-black text-slate-900 dark:text-white mb-8 md:mb-10 flex items-center gap-4 uppercase tracking-tighter italic">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-parlour-50 dark:bg-parlour-500/10 flex items-center justify-center text-parlour-600">
                <Building2 size={24} />
              </div>
              General Information
            </h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-8">
              {[
                { name: 'salonName', label: 'Salon Title', icon: Building2 },
                ...
                <div key={field.name} className="space-y-3">
                  <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2 flex items-center gap-2 italic">
                    <field.icon size={12} className="text-parlour-500" />
                    {field.label}
                  </label>
                  <input
                    name={field.name}
                    ...
                    className={`w-full bg-slate-50 dark:bg-slate-800 border ${formik.touched[field.name] && formik.errors[field.name] ? 'border-red-500' : 'border-slate-100 dark:border-white/5 shadow-inner'} p-4 md:p-5 rounded-2xl outline-none focus:border-parlour-500 font-bold text-slate-700 dark:text-white transition-all`}
                  />
              ...
              <div className="md:col-span-2 space-y-3">
                <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-2 flex items-center gap-2 italic">
                  <MapPin size={12} className="text-parlour-500" />
                  Operational Address
                </label>
                <textarea
                  name="address"
                  ...
                  className={`w-full bg-slate-50 dark:bg-slate-800 border ${formik.touched.address && formik.errors.address ? 'border-red-500' : 'border-slate-100 dark:border-white/5 shadow-inner'} p-4 md:p-5 rounded-2xl outline-none focus:border-parlour-500 font-bold text-slate-700 dark:text-white transition-all resize-none shadow-inner`}
                />
              </div>
            </div>
          </div>
          ...
          <div className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-2xl border border-slate-50 dark:border-white/5 shadow-2xl relative overflow-hidden group">
            ...
            <div className="space-y-3 md:space-y-4">
              {formik.values.businessHours.map((day, index) => (
                <div key={day.day} className="flex flex-col md:flex-row md:items-center justify-between p-4 md:p-6 bg-slate-50 dark:bg-slate-800/40 rounded-2xl border border-slate-100/50 dark:border-white/5 group hover:border-parlour-200 transition-all gap-4 md:gap-0">
                  <span className="font-black text-[10px] md:text-sm text-slate-700 dark:text-white uppercase tracking-tighter italic group-hover:text-parlour-600 transition-colors">{day.day}</span>
                  <div className="flex flex-col md:flex-row md:items-center justify-between md:justify-end gap-3 md:gap-6 w-full md:w-auto bg-white/50 dark:bg-slate-900/50 p-2 md:p-0 rounded-xl md:rounded-none">
                    <div className="flex items-center gap-2 md:gap-4 flex-1 md:flex-none">
                      <input
                        ...
                        onChange={formik.handleChange}
                        value={day.open}
                        className="flex-1 md:flex-none bg-white dark:bg-slate-800 px-2 py-1.5 md:px-4 md:py-2 rounded-xl font-black text-parlour-600 outline-none border border-slate-100 dark:border-white/5 text-[10px] md:text-xs shadow-sm min-w-0"
                      />
        ...
        <div className="space-y-6 md:space-y-10">
          <div className="bg-white dark:bg-slate-900 p-6 md:p-10 rounded-2xl border border-slate-50 dark:border-white/5 shadow-2xl group text-center flex flex-col items-center">
            ...
            <div className="w-32 h-32 md:w-48 md:h-48 rounded-2xl bg-slate-50 dark:bg-slate-800 border-4 border-dashed border-slate-100 dark:border-white/5 flex flex-col items-center justify-center text-slate-300 mb-6 md:mb-8 cursor-pointer hover:bg-parlour-50 transition-all group overflow-hidden relative">
              {formik.values.logo ? (
              ...
              <div className="absolute inset-0 bg-parlour-600/10 opacity-0 group-hover:opacity-100 transition-opacity" />
            </div>
