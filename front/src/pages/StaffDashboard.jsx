    ...
    <div className="space-y-6 md:space-y-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          ...
          <motion.div
            ...
            className="bg-secondary rounded-xl md:rounded-2xl p-4 md:p-6 border border-white/5 shadow-3xl relative overflow-hidden group hover:border-primary/20 transition-all duration-500"
          >
            <div className="absolute top-0 right-0 p-4 md:p-8 opacity-5 group-hover:opacity-10 transition-opacity">
              <stat.icon size={48} md:size={64} strokeWidth={1} />
            </div>

            <div className="flex items-center justify-between relative z-10 mb-4 md:mb-8">
              <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl bg-background flex items-center justify-center text-primary group-hover:bg-primary group-hover:text-secondary transition-all duration-500 shadow-premium border border-white/5">
                <stat.icon size={18} md:size={22} strokeWidth={2.5} />
              </div>
              <div className="flex items-center gap-2 text-[8px] md:text-[9px] font-black text-primary bg-primary/10 border border-primary/20 px-2 md:px-3 py-1 md:py-1.5 rounded-xl md:rounded-2xl uppercase tracking-[0.2em] ">
            ...
            <div className="relative z-10">
              <h3 className="text-muted text-[7px] md:text-[9px] font-black uppercase tracking-[0.4em] mb-1 md:mb-3  leading-none opacity-60 group-hover:opacity-100 transition-opacity">
                {stat.label}
              </h3>
              <p className="text-lg md:text-3xl font-black text-white tracking-tighter leading-none  font-luxury overflow-hidden truncate">{stat.value}</p>
            </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 max-">
        <div className="lg:col-span-2 bg-secondary/30 backdrop-blur-sm p-10 rounded-2xl border border-white/5 shadow-3xl relative overflow-hidden group">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 md:gap-6 mb-8 md:mb-16 relative z-10">
            <div>
              <h3 className="text-lg md:text-2xl font-black text-white tracking-tighter uppercase mb-2 md:mb-3  flex items-center gap-3 md:gap-4 font-luxury">
          ...
          <div className="h-[200px] sm:h-[300px] md:h-[400px] w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.revenueData}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fill: '#A0A0A0', fontSize: 7, fontWeight: 900, letterSpacing: '2px' }} dy={10} interval="preserveStartEnd" />
          ...
          <div className="space-y-6 flex-1 max-h-[300px] overflow-y-auto custom-scrollbar pr-2">
            {data.specializationData.map((service, i) => (
              <div key={i} className="space-y-2 group">
                <div className="flex justify-between items-end text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.4em] text-muted ">
                  <span className="text-white truncate max-w-[100px] sm:max-w-[160px] group-hover:text-primary transition-colors">{service.name}</span>
      ...
      <div className="bg-secondary/30 backdrop-blur-sm p-6 md:p-10 rounded-xl md:rounded-2xl border border-white/5 shadow-3xl relative overflow-hidden group">
        ...
        <div className="h-[200px] md:h-[300px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={data.occupancyData} margin={{ bottom: windowWidth < 768 ? 0 : 0 }}>
              <CartesianGrid strokeDasharray="8 8" vertical={false} strokeOpacity={0.03} />
              <XAxis 
                dataKey="hour" 
                axisLine={false} 
